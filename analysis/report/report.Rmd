...
errors <- local(
  {
    p <- progressr::progressor(steps = N_valid, enable = TRUE)

  map_func(seq_len(N_valid), function(test_index){
  ...
  if(params$generate_model_fits){
    ...
    walk(
      seq_along(tuning_parameters), ~agtboost::gbt.save(
        fits[[.x]],
        here::here(paste0("analysis/data/derived_data/fits/agt_", paste0(test_index, "_", paste0(unlist(tuning_parameters[[.x]]), collapse = "_")), ".agtb"))
        )
    )
  } else {
    fits <- map(
  ...
  #update progressr so it knows that this fold is finished
  p()
  #calculate error
