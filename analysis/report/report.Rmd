...
```{r get vaccine data}
iso3cs <- unique(combined_df$iso3c)
names(iso3cs) <- iso3cs
#Load when variant timings start, no source available generated when model
...
#vaccine efficacies per replicate
if(params$gather_raw_data){
  map(iso3cs, function(iso3c){
    dir.create(here::here("temp"))
    exists <- tryCatch(
    download.file(
        paste0("https://github.com/mrc-ide/covid-vaccine-impact-orderly/blob/main/data/excess_mortality/model_fits/", 
           iso3c, ".Rds?raw=true"), here::here(paste0("temp/temp.Rds")),
        mode = "wb"
      ),
    error = function(e){NULL})
    if(!is.null(exists)){
      fit <- readRDS(here::here("temp/temp.Rds"))
      unlink(here::here("temp"), force = TRUE, recursive = TRUE)
      #get ve per replicate
      list(
        ve = fit$replicate_parameters$ves,
        fit$interventions$vaccine_efficacies
      )
    } else {
      NULL
    }
  }) %>% 
    saveRDS(here::here("analysis/data/raw_data/excess_mortality_vpar.Rds"))
  map(iso3cs, function(iso3c){
    dir.create(here::here("temp"))
    exists <- tryCatch(
    download.file(
        paste0("https://github.com/mrc-ide/covid-vaccine-impact-orderly/blob/main/data/reported_deaths/model_fits/", 
           iso3c, ".Rds?raw=true"), here::here(paste0("temp/temp.Rds")),
        mode = "wb"
      ),
    error = function(e){NULL})
    ...
    saveRDS(here::here("analysis/data/raw_data/reported_deaths_vpar.Rds"))
}
vaccine_efficacies <- readRDS(here::here("analysis/data/raw_data/excess_mortality_vpar.Rds")) %>% 
  map_dfr(calculate_ve_map_func, .id = "iso3c") %>% 
  mutate(type = "excess") %>% 
  rbind(
    readRDS(here::here("analysis/data/raw_data/reported_deaths_vpar.Rds")) %>% 
    map_dfr(calculate_ve_map_func, .id = "iso3c") %>% 
    mutate(type = "reported")
  ) %>% 
  #apply breakthrough scaling
  mutate(
    replicate = as.numeric(replicate),
    ve_d_low = (ve_d_low - ve_i_low)/(1 - ve_i_low),
    ve_d_high = (ve_d_high - ve_i_high)/(1 - ve_i_high),
    ve_d_low_d = (ve_d_low_d - ve_i_low_d)/(1 - ve_i_low_d),
    ve_d_high_d = (ve_d_high_d - ve_i_high_d)/(1 - ve_i_high_d)
  )

#mixing matrix
mixing_matrices <- map(iso3cs, ~squire::get_mixing_matrix(iso3c = .x))
#convert into a tibble
mixing_matrices <- map_dfr(mixing_matrices, function(x){ 
  row <- as.numeric(x)
  names(row) <- paste0("mixing_matrix_", seq_along(row))
  row
}, .id = "iso3c")

#ISSUE: SOME OF THESE SHOULD GO IN A MATRICES!
#not sure mixing matrices belong, + make an effective protection matrix by variant/dose?
...
parameter_data <- deaths_averted %>%
  ...
  ungroup() %>%
  #mixing matrix
  left_join(mixing_matrices, by = "iso3c") %>% 
  #add vaccine efficacy, for each replicate for each variant
  left_join(vaccine_efficacies, by = c("iso3c", "replicate", "type")) %>% 
  #epidemic start date
...
vars_to_standardise <- parameter_data %>% select(is.numeric) %>% names()
...
if(params$generate_model_fits){
  ...
  unlink(here::here("analysis/data/derived_data/fit"), recursive = TRUE)
